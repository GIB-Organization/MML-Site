...
export class AdditionalDataFormService {
  ...
  form = this.fb.group<IAdditionalDataFormGroup>({
    subProfessionId : new FormControl(null, [Validators.required]),
    policyPeriod : new FormControl(null, [Validators.required]),
    experiencePeriodId : new FormControl(null, [Validators.required]),
    birthMonth : new FormControl(null, [Validators.required]),
    birthYear : new FormControl(null, [Validators.required]),
    malpracticeClaimDeclared : new FormControl(false, [Validators.required]),
    malpracticeClaimDetails : new FormControl(null),
    hasPreviousInsurance : new FormControl(false, [Validators.required]),
    previousPolicyNumber : new FormControl(null),
    previousPolicyEndDate : new FormControl(null),
    previousInsuranceCompanyCode : new FormControl(null),
    retroactiveDate : new FormControl(null),
  ...
  }, { validators: this.mmpDeclarationsValidation() })

  // get estimatedValue() {
  //   return this.form.controls.estimatedValue
  // }
  get subProfessionId() {
    return this.form.controls.subProfessionId
  }
  get policyPeriod() {
    return this.form.controls.policyPeriod
  }
  get experiencePeriodId() {
    return this.form.controls.experiencePeriodId
  }
  get birthMonth() {
  ...
  get malpracticeClaimDeclared() {
    return this.form.controls.malpracticeClaimDeclared
  }
  get malpracticeClaimDetails() {
    return this.form.controls.malpracticeClaimDetails
  }
  get hasPreviousInsurance() {
    return this.form.controls.hasPreviousInsurance
  }
  get previousPolicyNumber() {
    return this.form.controls.previousPolicyNumber
  }
  get previousPolicyEndDate() {
    return this.form.controls.previousPolicyEndDate
  }
  get previousInsuranceCompanyCode() {
    return this.form.controls.previousInsuranceCompanyCode
  }
  get retroactiveDate() {
    return this.form.controls.retroactiveDate
  }
  get isPoliticallyExposedPerson() {
    return this.form.controls.isPoliticallyExposedPerson
  }

  mmpDeclarationsValidation(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const GROUP = control as FormGroup<IAdditionalDataFormGroup>;
      if (GROUP.value.malpracticeClaimDeclared && !GROUP.value.malpracticeClaimDetails) {
        GROUP.controls.malpracticeClaimDetails.setErrors({ 'required': true });
      } else {
        GROUP.controls.malpracticeClaimDetails.setErrors(null);
      }
      const hasPrevious = GROUP.value.hasPreviousInsurance;
      const previousRequiredControls = [
        GROUP.controls.previousPolicyNumber,
        GROUP.controls.previousPolicyEndDate,
        GROUP.controls.previousInsuranceCompanyCode,
        GROUP.controls.retroactiveDate,
      ];
      previousRequiredControls.forEach(c => {
        if (hasPrevious && !c.value) {
